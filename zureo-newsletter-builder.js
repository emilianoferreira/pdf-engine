...
function estimarAlturaTextoMm(texto, opciones = {}) {
  ...
  lineasTexto.forEach((linea) => {
    const longitud = [..._segmenter.segment(linea.trim())].length || 1;
    const lineasEstimadas = Math.max(
      1,
      Math.ceil((longitud * avgCharWidthMm) / maxWidthMm),
    );
    totalLineas += lineasEstimadas;
  });

  const separacionEntreParrafosMm =
    Math.max(0, lineasTexto.length - 1) * pxToMm(4);
...
body.forEach((elemento) => {
  ...
  const textoRaw = elemento.paragraph.elements
    .map((e) => e.textRun?.content || "")
    .join("");

  // --- DETECCIÓN DE PÁRRAFOS VACÍOS ---
  const tieneImagen = elemento.paragraph.elements.some(
  ...
  if (
    ["HEADING_1", "HEADING_2"].includes(estiloDoc) ||
    textoRaw.startsWith("#")
  ) {
    ...
    else if (/\bON\b/.test(upper) || upper.includes("ZUREO ON"))
      sistemaEncontrado = "on";
  }
  ...
  const textoParrafoLimpio = limpiarTextoGeneral(textoRaw)
    .replace(/\n/g, "")
    .trim();
  ...
  if (regexFuncParrafo.test(textoParrafoLimpio)) {
    ...
    const nombreFuncionalidad = textoParrafoLimpio
      .replace(regexFuncParrafo, "")
      .trim();
  ...
  elemento.paragraph.elements.forEach((el) => {
    ...
    if (el.inlineObjectElement) {
      ...
      if (imagenesDoc[id]) {
        const embeddedObj =
          imagenesDoc[id].inlineObjectProperties.embeddedObject;
  ...
  if (textosAcumulados.trim() || imagenesAcumuladas.trim()) {
    ...
    if (textosAcumulados.trim()) {
      ...
      if (esDetalle)
        textoFormateado = `<span class="det-txt">${textoFormateado}</span>`;
